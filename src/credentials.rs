...
impl KeyringCredentialStore {
    ...
    pub fn new(master_password: String) -> Self {
        ...
        // Try to use keyring, fallback to file storage if not available
        let file_fallback = match keyring::Entry::new(&service_name, "test") {
            Ok(entry) => {
                // Test if we can actually use the keyring
                match entry.set_password("test") {
                    Ok(_) => {
                        let _ = entry.delete_credential(); // Clean up test
                        None // Keyring is available
                    }
                    Err(_) => {
                        warn!("Keyring not available, falling back to file storage");
                        FileCredentialStore::new(master_password).ok()
                    }
                }
            }
            Err(_) => {
    ...
    fn get_from_keyring(&self, id: &CredentialId) -> Result<Option<StoredCredential>> {
        ...
        match entry.get_password() {
            Ok(json_data) => {
                let credential: StoredCredential = serde_json::from_str(&json_data)?;
                Ok(Some(credential))
            }
            Err(keyring::Error::NoEntry) => Ok(None),
            Err(e) => Err(anyhow!("Failed to retrieve from keyring: {}", e)),
        }
    }
}
...
impl CredentialStore for KeyringCredentialStore {
    fn store_credential(
        ...
        if let Some(ref mut file_store) = self.file_fallback {
            return file_store.store_credential(name, description, credential);
        }
        ...
        self.store_in_keyring(&id, &stored_credential)?;
        info!("Stored credential in keyring with ID: {}", id);
        Ok(id)
    }

    fn get_credential(&self, id: &CredentialId) -> Result<Option<StoredCredential>> {
        if let Some(ref file_store) = self.file_fallback {
            return file_store.get_credential(id);
        }

        self.get_from_keyring(id)
    }

    fn list_credentials(&self) -> Result<Vec<CredentialSummary>> {
        if let Some(ref file_store) = self.file_fallback {
            return file_store.list_credentials();
        }

        // Note: Keyring doesn't provide a way to list all entries
        // In a real implementation, we'd need to maintain an index
        warn!("Keyring credential listing not fully implemented");
        Ok(Vec::new())
    }
    ...
    fn update_credential(
        ...
        if let Some(ref mut file_store) = self.file_fallback {
            return file_store.update_credential(id, name, description, credential);
        }

        // Get existing credential to preserve timestamps
        if let Some(mut stored_credential) = self.get_from_keyring(id)? {
            stored_credential.name = name;
            stored_credential.description = description;
            stored_credential.credential = credential;
            self.store_in_keyring(id, &stored_credential)?;
            info!("Updated credential in keyring with ID: {}", id);
            Ok(())
        } else {
            Err(anyhow!("Credential with ID {} not found", id))
        }
    ...
    fn delete_credential(&mut self, id: &CredentialId) -> Result<()> {
        if let Some(ref mut file_store) = self.file_fallback {
            return file_store.delete_credential(id);
        }

        let entry = self.get_keyring_entry(id)?;
        entry
            .delete_credential()
            .map_err(|e| anyhow!("Failed to delete from keyring: {}", e))?;
        info!("Deleted credential from keyring with ID: {}", id);
        Ok(())
    }

    fn mark_credential_used(&mut self, id: &CredentialId) -> Result<()> {
        if let Some(ref mut file_store) = self.file_fallback {
            return file_store.mark_credential_used(id);
        }
...
mod tests {
    ...
    #[test]
    fn test_file_credential_store() -> Result<()> {
        let temp_dir = TempDir::new()?;
        let storage_path = temp_dir.path().join("test_credentials.enc");

        // Create a store with a test password
        let master_password = "test_password_123".to_string();
        let mut store = FileCredentialStore {
            storage_path,
            master_password: SensitiveString::new(master_password),
            credentials: HashMap::new(),
        };
        ...
        let id = store.store_credential(
            "Test Credential".to_string(),
            Some("A test credential".to_string()),
            credential,
        )?;
        ...
        assert_eq!(retrieved.name, "Test Credential");
        assert_eq!(retrieved.description, Some("A test credential".to_string()));

        // Test listing credentials
        let summaries = store.list_credentials()?;
        assert_eq!(summaries.len(), 1);
        assert_eq!(summaries[0].name, "Test Credential");
