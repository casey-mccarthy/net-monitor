...
fn test_database_persistence() {
    ...
    let http_node_retrieved = nodes2.iter().find(|n| n.name == "Integration Test HTTP Node").unwrap();
    let ping_node_retrieved = nodes2.iter().find(|n| n.name == "Integration Test Ping Node").unwrap();
    
    assert_eq!(http_node_retrieved.id, Some(http_id));
    assert_eq!(ping_node_retrieved.id, Some(ping_id));
    
    if let MonitorDetail::Http { url, expected_status } = &http_node_retrieved.detail {
    ...
    if let MonitorDetail::Ping { host, count, timeout } = &ping_node_retrieved.detail {
